...
| Ollama Cloud | — <sup>12</sup> | ✅ <sup>13</sup> | — <sup>12</sup> | — <sup>12</sup> | — <sup>12</sup> |
...
13. The Ollama session and weekly meters need an API key (`OLLAMA_API_KEY`, or `agents.ollama.api_key_file`) and hide themselves if it is missing or rejected. Export the key (`export OLLAMA_API_KEY=...`), rather than merely assigning it: an unexported shell variable is visible in the terminal but is not inherited by the companion. A running shell keeps its old environment, so re-source your profile or open a new shell before relaunching; alternatively set `agents.ollama.api_key_file` in `~/.config/codelight/config.json`. Ollama's usage endpoint reports no reset timestamp for either window, so both bars show an empty reset (`↻ --` in the panels, `(resets --)` in VSCode); the 4-week range it does report is an activity window, not a quota reset.
</sub>
